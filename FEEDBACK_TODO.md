...
- [ ] ⏳ SCOTT on v1.21.47: feel + pill jank number + one [pace] line mid-run. If jank still high → the remaining lever is REAL THREADS via coi-serviceworker (audio off main thread entirely; test PWA carefully). If audio sounds too dull at 22k → revert mix_rate alone.
