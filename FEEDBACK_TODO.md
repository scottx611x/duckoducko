...
- [ ] ⏳ AWAITING SCOTT on v1.21.44: (1) SETTINGS footer "gpu:" line — if it says SwiftShader/llvmpipe that's the whole answer; (2) SHOW FPS pill numbers — menu vs mid-run vs boss, steady-low vs spiky. NO more blind perf theories — next fix chases these numbers. (If SwiftShader: the fix is likely WebGL flags/canvas attributes or accepting APK as the Android path. If real GPU + steady-low: gate shimmer/wash on web, thin outlines. If spiky: .filter() churn pooling, audio main-thread mixing.)
