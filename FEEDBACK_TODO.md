...
- [ ] ⏳ SCOTT'S FEEL-VERDICT on v1.21.46: does it still feel laggy in actual play? If yes → next lever is THREADS via coi-serviceworker on Pages (moves audio off the main thread; needs careful PWA/cache testing before shipping — get sign-off first). If transitions-only → possibly acceptable / target death-screen texture warms instead.
