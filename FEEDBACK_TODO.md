...
- [x] **Sadie model de-wonked (profile + back + poses)** v1.21.39 (Scott: "sadie boss model is still a bit wonky... hunchback/wonkiness" + "she is just a bit wonky looking") — three real defects found by rendering her at every angle: (1) HUNCHBACK: the seated torso peaked at the REAR with the head floating forward and no neck — apex moved forward, a rear taper added, and a real NECK column now bridges shoulders→skull (profile/back read as a proper seated lab); (2) pounce/run "flying ears" were single-voxel diagonal chains that rendered as detached floating dots — now solid sheets streaming back off the skull; (3) the play-bow rump was a disconnected ball towering over the dropped head — now high-but-connected via a sloping back. Regenerated EVERYTHING downstream: boss set, wardrobe poses, codex turntable, playable-sadie menus, gallop frames, and the exotic wear melds re-anchored to the new head. 🟡 eyeball her in the boss fight + wardrobe.
- [x] **TRUE wearable melds for Rusty + Sadie & swim-model Sadie** v1.21.38 (Scott: "wearables don't sit properly on sadie or rusty" + "I like sadie's existing swimming model with chuckit for in-game" + "if you are sadie you should never see another sadie") — (1) the 2D nudge is gone: tools/gen_exotic_wear.py TRANSLATES every hat/bodywear from the mallard's head to each exotic model's actual head, occludes against THEIR body, at THEIR scale — full sets (idle/banks/sides/hero/24-spin) named wear3d_<species>_<id>_*, loaded via a species-aware _load_wear3d, PLUS baked MEGA hat-stack slices so the tumble stays welded; (2) playable Sadie's in-game frames are now THE swimming model — chuckit in mouth, head/shoulders/tail out of the water, back view, and her MEGA stack tumbles the swim model ball-and-all (menus keep the seated girl); (3) Sadie-as-Sadie sees NO other Sadie: ambient cameo gated off and a rolled MEGA SADIE final silently becomes GERALD THE ETERNAL. 🟡 dress them up and eyeball.

## 🐌🐌 PLAYTEST (Scott, 2026-07-12, on v1.21.42): "woah this is even worse"
- [x] **REVERTED viewport stretch** v1.21.43 — the fill-rate hypothesis was falsified on the device that matters; back to canvas_items (renderer state = pre-pass). KEPT the water/bank one-call tiling (strictly fewer commands, shotsweep-verified pixel-identical). LESSON: viewport stretch on Godot web can be SLOWER on real phones (offscreen target + blit path) — do not re-apply without on-device numbers.
- [x] **SHOW FPS toggle** v1.21.43 — settings toggle (persisted); bottom-left pill: live fps + "worst N" (worst frame of each rolling 3s window, so hitches are visible, not just averages). New --settingsshot harness. Settings rows re-spaced to fit (toggles 46px pitch).
- [x] Browser known: **Android Chrome**. → v1.21.44 adds the SwiftShader detector: settings footer now shows "gpu: <adapter>" (Chrome blocklists some Android GPUs and software-renders WebGL — would explain everything).
- [x] SCOTT'S SCREENSHOT (v1.21.44, settings): **"60 fps · worst 9"** + gpu: "WebKit WebGL" (masked). THE PROFILE IS SPIKY, NOT SLOW: full 60 average with ~110ms stalls — even on the near-empty settings screen. Fill rate/draw calls were never the problem (explains why the viewport-stretch "fix" only hurt). The hunt is now for main-thread STALLS.
- [x] **Diagnostics + cache fixes** v1.21.45 — (1) HITCH TRACER: with SHOW FPS on, any frame >80ms logs context to VIEW APP LOGS ([hitch] ms, screen, entity counts, boss, last save/lazy-art-load breadcrumb — _save/_load_wear3d/_flapspin stamped); (2) UNMASKED GPU via JavaScriptBridge (WEBGL_debug_renderer_info) — settings footer now shows the real Adreno/Mali/SwiftShader name, not "WebKit WebGL"; (3) update-banner bug FIXED: the version.txt cache-buster used ticks_msec which repeats every boot → Chrome served one cached version.txt forever (why no banner) — now wall-clock, AND re-pings on menu return (5-min throttle) for long-lived PWA processes; (4) **versioned pck**: web deploys rename index.pck → index-v<ver>.pck + mainPack in GODOT_CONFIG — a stale html can never load a mismatched pck again (verified end-to-end in headless Chrome).
- [x] SCOTT'S HITCH LOGS (v1.21.45, two screenshots) — gpu = REAL hardware (PowerVR DXT-48-1536, ANGLE ES 3.2 — SwiftShader ruled out). Findings: (a) save-adjacent stalls (run start / death / menu: io=save 0.1-1.1s-ago, 118-140ms); (b) flapspin:mallard lazy load = 142ms settings hitch; (c) THE BIG ONE: sustained ~130ms stalls EVERY 0.6-0.9s for 20+ seconds (t=111-132, scr=play, logs=4, everything else 0, no recent io) — a metronome of freezes with an empty river; cause NOT yet visible to tracer v1.
- [x] **v1.21.46: evidence-backed fixes + tracer v2** — (1) _codex_see no longer saves MID-RUN (a full save on "first time seeing a snack" = guaranteed phone stall); defers via save_dirty, flushes at death/menu (both already save); (2) menu duck's 24 flap frames preload at boot (was the 142ms settings hitch); (3) TRACER v2: hitch lines now split the stall — p2d (script/physics) vs d2n (canvas render + audio mix + GC + engine) — plus enemies/items/sfx-per-second counts and dead/tut/draft/pause screen tags. Whatever the 0.7s metronome is, the next log names its side of the fence.
- [x] SCOTT'S v1.21.46 LOG — the fixes LANDED: the 0.7s metronome is GONE; a 47-second run logged ONE hitch (at a genuinely busy moment: parts=17 sfx=8, p2d=60 d2n=60). Remaining stalls are ENGINE-SIDE (p2d≈0-3, d2n=80/229/320/3744) and cluster at TRANSITIONS (run start, death screen, settings). d2n=3744 at 12.9s looks like an app-switch/screenshot gesture, not the game. Verdict: script is innocent; the residue lives in engine land (texture upload / audio mix / IDBFS sync at save moments / possible first-use shader compiles on PowerVR).
- [x] SCOTT on v1.21.46: "still pretty laggy" + tracer v2 log: p2d≈0 / d2n=80-320 (engine-side), metronome gone, only isolated 80ms+ hitches → the FEEL is a carpet of sub-threshold (30-60ms) frames. → **v1.21.47**: (1) web canvas capped at DPR-1 (web_meta injects devicePixelRatio override + `image-rendering: pixelated` — the fill-rate cut WITHOUT viewport-stretch's extra pass; ~7-9x fewer pixels on his phone); (2) audio mix_rate 44.1k→22k (halves main-thread DSP; no-threads build mixes inside the frame); (3) pill adds "jank N" (>33ms frames/3s) + [pace] 10s census (avg ms, j33, j50) in app logs.
- [x] SCOTT on v1.21.47: **"that feels much better"** 🎉 — the carpet-jank read was right: DPR-1 canvas + 22kHz audio landed it. (Perf saga: the wins were codex-save-defer + DPR-1 fill cut + audio DSP halved; the losses were both resolution theories shipped without device data. Rule stands: measure on the failing device first.)
- [ ] (optional, dormant) if lag ever creeps back: [pace]/jank metrics are in the pill; next lever remains coi-serviceworker threads.

## 💭 DESIGN (Scott, 2026-07-19): "the splits are still feeling a bit ho-hum — what would you suggest"
- [x] **v1.22.0: THE LIVING FORK** — the modal chooser is GONE; the river actually splits again. A real voxel island (grass spine, rocky shore, leaning pine, stump) scrolls in; the Ancient Duck PRESIDES from it with gold motes; channel banners hang over each mouth (pond + branch + deal, your side gold-rings as you steer); each channel mouth is pre-washed in its branch water so both futures are VISIBLE; the current eases to reading speed; where you ARE when the nose reaches you is your water. Sky route = a labeled golden thermal column over the island — hold center to ride it. World never stops. + **THE STRANGE WATER**: ~16% of forks hide one channel behind "? ? ? ? ?" (reveals on commit). + **THE RIVER REMEMBERS**: 2+ danger picks in a run earn THE GAUNTLET (3x heron packs, narrows pinch, blood-dark water; unhit = +100/+shield/LOFT); 2+ calm picks earn LUCIEN'S COVE (calm rules + Lucien hums at the bank + a mid-stretch gift). Verified: filmstrip (full approach→steer→commit→ride→arrival), botsim 4-run, gauntlet branchshot, shotsweep 2x9/9 (staged fork-free — random forks were ambushing the harness). Dead chooser-UI code left unreachable, cleanup later. 🟡 THE feel pass.
- [x] **"the jetskis look like trash"** v1.22.0 — rebuilt: long sleek two-tone runabout (colored hull + pale deck, real stepped saddle, console + raked bars with grips, jet pump, waterline lap), still moored afloat per canon. Verified zoomed.

## 🎮 PLAYTEST — KEZ, first external tester! (Discord, 2026-07-14): "Dude that's pretty fun. I just wish it would let me tap with my other finger to jump while my first is dragging side to side"
- [x] **TRUE MULTITOUCH** v1.21.54 (Scott: "your wish is my command") — a second finger was being swallowed by the pinch guard. Now: while one thumb drags to steer, ANY other finger tap = instant HOP (fires on press, not release — snappier than a normal tap). The steer drag survives helper-finger lifts, and if the STEERING finger lifts first the other finger takes over seamlessly (anchor handed off, no lurch, no accidental hop). Pinch-zoom still owns two fingers on the logbook graph; menus/choosers unaffected. 🟡 needs REAL-DEVICE feel check (Kez is the perfect tester).

## 💭 DESIGN (Scott, 2026-07-13): "pretty close with the lane splits... honestly something feels a bit missing"
- [x] **BRANCH WATERS** v1.21.53 ("sick, dope, love it send it") — every wager is now a PLACE layered on the pond (canon untouched — branch = weather, not geography): **THE NARROWS** (DANGER PAYS: the banks PHYSICALLY close in — bankw is now a variable, river ~25% narrower, gameplay clamps + shore art + reed waterline all move together; dusk wash + hunting shadows sweep the water) · **THE SHALLOWS** (FEAST: sun wash, bug-hatch specks, a frog on nearly EVERY log — 0.95 spawn) · **THE BACKWATER** (SCAVENGER: tea-stained water) · **THE STILLS** (QUIET: mist bands + fireflies + the music drops to a low-pass hush — new Music bus) · EASY MONEY gets drifting down. + timeline shows the NEXT SPLIT as a pulsing gold Y (plan for it, not ambushed) + the road not taken drifts away ("easy money drifts away..."). Verified: --branchshot harness per branch (Narrows pinch reads great), botsim 4-run clean. 🟡 feel pass is yours.
- [x] **"after choosing a path my duck is friggin rushed through to go past the signs"** v1.21.53 — the ride is now TWO beats: a gentle glide (1.15x) where she drifts into her channel and passes UNDER her sign at reading speed (sign gold-pulses, knocks on its post + chime + ripple as she passes), THEN the 2.4x whitewater rush. Sign hustle-off only after the rush begins. FOUND + FIXED via filmstrip: the longer glide gave logs time to spawn in and KILL her mid-cinematic ("rude." — literally) — she's on rails now: invincible during the ride + log spawns held. 🟡 feel.

## ➕ PLAYTEST (Scott, 2026-07-12, on v1.21.49): "woah the text on the web build is all hard to read now"
- [x] v1.21.50 — the DPR-1 cap's cost: vector text rendered at ~412px-wide backing then nearest-upscaled by a FRACTIONAL ~2.6x = ragged staircase glyphs. Now HALF device resolution (devicePixelRatio/2, min 1): the compositor upscale is exactly 2x (clean integer blocks), text gets ~30% more pixels, fill still ~4x cheaper than the original laggy full-DPR build. 🟡 Scott checks BOTH: text readable? perf still good? If lag returns → the dial sits between /2 and 1; if text still rough → next step is bumping small font sizes on web.

## ➕ PLAYTEST (Scott, 2026-07-12, on v1.21.47)
- [x] **"Rusty's empowering chats are so bad lol"** v1.21.49 — the 22 cheerleader HAWK_LINES are GONE ("the river's lucky to have you, kid." — no more). Rusty is now a grizzled red-tail with a predator's manners: deadpan, faintly threatening, secretly fond, occasionally useful ("i eat things your size, you know. fly like you remember that." / "hold that thought — vole. 400 feet. ...it's fine. it knows what it did." / "SKREE. ...apologies. force of habit when something small moves." / "you want empowerment, find a loon. you want ALTITUDE, watch me."). Thermals low-grade sendoff de-sapped ("i've seen LEAVES do better"), snack-chain advice line dried out. hawk_pool_v=2 save flag deals the new rotation fresh (old seen-indices would've hidden most of it). The psychic history reads (three-deaths-to-same-killer coaching etc.) kept — those were already in-voice. 🟡 VOICE CHECK: Scott judges the new tone; happy to punch up/darken further.
- [x] **"Rusty's phrases are still a little weird"** v1.21.51 — the deadpan pass was OVERWRITTEN: two-line comedy bits with punchlines (vole gags, merganser gossip, "find a loon"). The game's actual register is terse ("well.", the frog's flat "ribbit."). Pool rewritten again: 16 SHORT dry lines, real airmanship, zero bits ("steer before the log. not at it." / "hop less. mean it more." / "watch the shadow, not the bird." / "slow is smooth. smooth is fast." / "still alive. good. keep doing that."). Thermals low-grade line tightened the same way ("the wind won this one. again."). hawk_pool_v=3 re-deals. 🟡 tone re-check.
- [x] **"Rusty should make folks happy. he is the wise sage of the mountain but he's also a lil kooky"** v1.21.52 — THE BRIEF, at last, in Scott's words (canon now). Terse pilot OUT. Rusty = warm mystical guru, earnest and slightly unglued: "i asked the moon about you. she's rooting for you. we all are." / "secret of the mountain: there is no mountain. only UP." / "i dreamed of a duck last night. it was you. you were magnificent." / "the log is your teacher. a terrible one. hop the teacher." / "when in doubt, spiral. works for hawks. works for galaxies." / "the sun sets, the sun rises. between the two: SNACKS." Thermals low-grade + snack-chain lines matched ("the wind is a fussy teacher. we'll charm her yet"). hawk_pool_v=4. Voice journey COMPLETE(?): jerk → nice → cheerleader → comedian → drill instructor → KOOKY MOUNTAIN SAGE. 🟡 tone verdict.
- [x] **Boss HP bar covers Bongo** v1.21.48 — Bongo floats at y=150, face-first into the y=112 backplate (any top-center boss had this). The duplicated bar code is now one _draw_boss_hpbar(), and it GHOSTS to 34% alpha whenever the boss drifts under it — the character always reads through the chrome. Verified via new `--bossshot --kind=<boss>` harness (bongo shot: face fully readable, pips still legible). 🟡 eyeball in a real duel.

## 🐌 PLAYTEST (Scott, 2026-07-11, on v1.21.41): "the live site is tremendously laggy on mobile"
- [x] **Mobile optimization pass** v1.21.42 — measured first (new `--perfshot` harness: avg frame ms + draw calls + process ms over 300 live frames). Desktop was never the problem (2ms frames); the mobile killers were: (1) **RENDER RESOLUTION — the big one**: stretch mode `canvas_items` makes phones rasterize at full device resolution (a DPR-3 phone = ~9x the pixels of the 540x960 design) through 4-5 stacked full-screen layers (water + shimmer + biome wash + scenery + vignettes) — brutal WebGL fill rate. Now `viewport` stretch: everything renders at 540x960 and upscales nearest — for pixel art this is visually the correct mode anyway (text gets the chunky pixel look, consistent with the font). (2) **water/bank tiling**: 64x64 water tile drawn in x/y loops = ~350 canvas commands per frame before entities; now ONE tiled draw_texture_rect per layer (texture_repeat enabled node-wide; theme-change sweep keeps a per-ROW fallback for its gradient). Draw calls 102→72, canvas rebuild much lighter for WASM. Verified: full --shotsweep (9 screens — water/banks/shimmer/boss arena pixel-identical), botsim 2-run smoke incl. a full 30k campaign run. 🟡 THE REAL TEST IS YOUR PHONE — if it's still rough after this, next levers are: gating the shimmer layer + wash on web, thinning draw_string_outline sizes, and pooling the per-frame array .filter() churn.

## 💡 IDEA (Scott, 2026-07-11): log-bounce power-up — "send it bruh"
- [x] **FROG LEGS** v1.21.41 (Scott: "bounce off of any log to chain together jumps") — epic (gold) draft power-up, skill-gated so it never becomes a flight mode: the bounce fires only when you COME DOWN on a log (descending half of a hop, or a 0.12s post-splash grace window — needed because the airborne threshold makes the pure in-hop landing window ~5% of the arc); walking into a log sideways still bonks. Chain decays honestly: each bounce starts deeper into the arc AND lifts less (0.86^n, floor 0.55) — 3-5 boings through a dense field, water resets the ledger. Pays 2×chain feathers + loft + ON FIRE heat, recharges DOUBLE HOP, ducklings boing along, and a resident frog LEAPS off cheering RIBBIT!! (codex frog credit). Herons still own the sky = the natural counter, untouched. New voxel icon (a frog mid-leap — reads delightfully swole), POWER_LORE entry (frog tuition ledger), synergy web (spring/double/springy/bounce), bot draft weight 1.4, new --bounceshot harness (proved chain=1, +2 feathers, re-airborne; screenshot shows BOING!/RIBBIT!!/ripple/HUD icon). Botsim 4-run regression clean. 🟡 the chain FEEL is yours — is 3-5 the right flourish length?

## ➕ PLAYTEST (Scott, 2026-07-10, on v1.21.39) — ALL 3 SHIPPED v1.21.40
- [x] **ANY trash power-up should let you collect trash** v1.21.40 — SCRAP SHELL (and DUMPSTER DIVE, same defect) couldn't stock the hoard they consume: every "flotsam floats reachable + magnets in + collectable ring + hoard HUD" gate checked only trashmag/packrat/junker. New single source of truth `_junk_boon_on()` covers all FIVE junkyard boons at all four gates. Smoke: botsim batch drafted scrapshell+dumpster+trashmag, ran clean. 🟡 grab a scrap-shell build and watch the orbit fill.
- [x] **THE SKY ROUTE flyover: boring + not my duck** v1.21.40 — "not my duck" was TWO real bugs: (1) _draw_sky rendered `ducks[species]` not `_eff_species()` — hens soared as their DRAKE (now hen sprites + hen tint); (2) a misplaced `else:` paired with the ducklings-check, so ANY duck with zero ducklings got a flat YELLOW DISC drawn over it (now strictly an asset-missing fallback). Boring fixed by giving the flyover a story: the clouds PART mid-glide and the duel you're skipping slides by far below — the actual boss, tiny and furious ("?!") on a toy river — while your duck talks trash down at it (per-boss quips; Sadie gets kindness: "next time, sweet girl! PROMISE!"); 6 golden feathers ride the thermal and sweep up as you pass (+2 each, rising chimes, folded into the landing payout); a full victory BARREL ROLL ("wheee!") before the descent; flight extended 2.8s→3.6s to let the beat land. Verified via --skyshot (boss gap + quip + feathers + fit all render). 🟡 fly one.
- [x] **The little x-multiplier text on every collected thing is annoying** v1.21.40 — the chain now lives in the AUDIO (rising combo scale, unchanged); "x5!" "x10!" text only salutes milestone multiples of 5. The per-snack "x3! x4! x6!..." spam is gone.
...
- [x] **Split sequence: the FULL cinematic pass** v1.21.33 (Scott: "the splits and the split transition is still bad" — so I filmed it: new --forkshot --film captures the whole choose→ride→arrive sequence as 26 frames) — the whole beat is now staged: (1) ENTRANCE: dim fades in, the Ancient Duck SWOOPS down with an overshoot bounce trailing gold motes, title drops, cards DEAL IN staggered from the bottom; (2) THE PICK: tapped card pops +6px with a pulsing gold ring while the other card dims and bows out — a 0.4s "locked in" beat before anything moves; (3) THE RIDE: she carves a back-eased line that overshoots and settles (not a linear slide), white water-streaks rush the whole river, cool blue tint, rooster-tail spray; (4) ARRIVAL: theme wash pre-rolled so the new pond is already washing in when the banner slams, and the old channel signs HUSTLE off stage (they used to linger, still advertising the pond you rejected). Filmstrip-verified frame by frame. 🟡 the real feel test is yours.
- [x] **Post-split transition: THE CURRENT TAKES HER** v1.21.32 (Scott: "transition is horrible and too slow... make this piece feel fun whimsical and exciting, same for the rewards") — ROOT CAUSE of the slowness: an old 0.45x "ease through the unpicked split" speed penalty kept strangling the river for seconds after you chose. Now: choose a water → she hops once with a quack → THE CURRENT GRABS HER: a ~1.1s whitewater rush at 2.4x speed, rooster-tail wake spraying, banking hard into the chosen line — and SLAMS into the arrival (banner + splash + gold burst + screen bump), signs whooshing past. Under 2 seconds, zero dead water. REWARDS deepened the same way: (1) LIVE CONTRACT CHIP on the HUD all stretch — "THE FEAST · 7/12", "DANGER PAYS · UNHIT — hold on!" (greys to "the toll is due" when tagged), "SCAVENGER · 5 junk = +10" ticking up live; (2) settlements are JACKPOTS — rising slot-machine chime arpeggio, gold burst, ripple ring, big contracts get the 6-note version. 🟡 feel.
- [x] **SPLITS have GRAVITAS: the wager system** v1.21.31 (Scott: "the choices lack gravitas/risk/reward/meaning") — every branch is now a CONTRACT stated plainly on its card and SETTLED with a verdict banner at the far end: DANGER PAYS (herons hunt 2.2x — finish unhit: +60 feathers +1 shield; get hit: "the herons collected their toll"), THE FEAST (snacks 2x — eat 12 before it ends: +30 + LOFT filled), SCAVENGER'S RUN (every junk grabbed pays double at the end), EASY MONEY (+40 flat), BLESSED (+25), THE QUIET WATER (explicitly: "nothing hunts here. nothing pays, either."), THE TRIAL (thermals). Cards now show the deal itself, not flavor text. 🟡 feel.
- [x] **Split run-up has SPUNK** v1.21.31 (Scott: "no spunk or whimsy") — choosing a water now triggers the elder's blessing (gold ripple ring + sparkle burst + "go well, little one.") and the duck SKIP-HOPS her way down the chosen leg, quacking, kicking up wake spray — an eager little traversal instead of a silent slide.
- [x] **Rogue Rusty at Weir Hill CULLED** v1.21.31 (Scott) — the automatic Cochichewick flyover is gone (it also stopped blocking boss spawns on that water). Rusty now appears only where he belongs: the early-run tip, his shop, and THE TRIAL.
- [x] **"Live site only has 1.21.24" — ROOT-CAUSED + update banner** v1.21.30 — the site was NOT stale: gh-pages + the Pages build + the served index.pck were byte-identical (md5-matched) with the local v1.21.29 build. Your DEVICE was serving a cached copy (second time this has bitten us; there is no service worker — it is plain browser/PWA caching of the 13 MB pck). Permanent fix: every deploy now writes version.txt, and the web build pings it at boot — when a newer build is live, the main menu shows a pulsing gold banner: "vX.Y.Z is OUT — close this tab & reopen to update." Stale caches announce themselves now. ✅ verifiable next release.
- [x] **NEXT SPLIT dev button** v1.21.29 (Scott: "dev menu thing to jump to next river split, or does next pond do that?" — it didn't: NEXT POND moves scenery/theme; forks ride their own distance clock) — dev menu now has NEXT SPLIT: the warning calls out and the split rounds the bend within seconds, chooser and all.
- [x] **DUCKLING MAGNET restored + the REAL green thing removed** v1.21.28 (Scott: "woah I like duckling magnet bro... I meant the green thing at the river forks") — DUCKLING MAGNET is fully back (pool, synergies, lore, Rusty tip, bot weights, dev kit — smoke-verified synergy(school)=true; sorry, bud). The actual "tractor beam" was the fork ISLAND: a giant flat solid-green triangle splitting the river that read like a beam projected on the water. GONE — the split is now told by the Ancient Duck's chooser + the two hanging channel signs (like real pilings), and the water stays water. The old push-off-the-grass code went with it.
- [x] **GIVE UP: "paddle ashore"** v1.21.27 (Scott's 302,602 ft "broken" run — that was ENDLESS mode, which had no exit but death) — the pause menu now has "paddle ashore · end run": banks feathers/logbook honestly like any death, killer credit reads "bested by your own good judgment."
- [x] **RIVER SPLITS: THE ANCIENT DUCK CHOOSER** v1.21.27 (Scott: "not clear what these actually do... rather stop and have the ancient duck give the choices clearly then animate the traversal... better risk/reward and clarity around boss skips") — the river now HOLDS at every split: the Ancient Duck descends in a spotlight ("the river SPLITS, little one. choose your water.") over two big cards — pond name, branch name, honest RISK/REWARD tag (SAFE / RISK & REWARD / TRIAL / BLESSED / JUNK...) and what it means. Pick a card and the duck paddles HERSELF down the chosen leg (the traversal, animated). BOSS SKIP is now a deliberate choice: when a boss looms within ~2600 ft, one card can be THE SKY ROUTE — "a thermal lifts you clean OVER the next boss · SKIP THE DUEL · forfeit its power board" — which flies the existing sky cinematic. Bot picks sides too (verified: full campaign win through the new chooser). 🟡 feel.
- [x] **Tractor beam REMOVED** v1.21.27 (Scott: "should remove the tractor beam thing too") — DUCKLING MAGNET (the power whose whole gimmick was tractor-beaming snacks across the river to your brood) is retired from the pool, its cross-river reel-in code deleted, synergy/bot tables cleaned. CRUMB MAGNET's short gentle pull stays (standard genre pickup). Also deleted: the old CLOUD GEYSER beam-up (dead code — it could never spawn; its sky cinematic lives on as THE SKY ROUTE). ❓ if "tractor beam" meant something else, name it and I'll hunt that instead.
- [x] **Hen MEGA hop now truly 3D** v1.21.26 (Scott: "hen mega jump is not the same 3dified variant as drakes") — hens were silently falling back to the FLAT sprite during MEGA because no hen stack slices were ever rendered (the old code comment even admitted it: "hen variants keep their OWN look"). All 14 hen variants now have real voxel sprite-stacks (tools/regen_hen_stacks.py; generate_ducks synced for future regens); slice counts match the drakes exactly so worn hats interleave at the same heights. Hens tumble identically to drakes now. 🟡 eyeball a hen MEGA.
- [x] **Codex DETAIL views blank for whole categories — FIXED** v1.21.25 (Scott: "it's the individual detail view... lots are missing") — the detail renderer's fallback chain had NO branch for three whole categories, so they fell into the flotsam path and drew an empty disc: THE SHORELINE (every fixture — campfire, owls, cow, pontoon, anglers, jetskis, Lizzie, landmarks...) now shows its actual bank sprite writ large; SHRINE BOONS (never had icons anywhere) get a gilded sigil disc with the boon's initial; SPECIALS get a proper big letter-disc. New harness: --codexshot saves a detail screenshot per category (/tmp/codex_detail_*.png) so blank details can't slip through again. 🟡 flip through the codex.
...
## ➕ ASK (Scott, 2026-07-28): "dishing out powerups too easily — want mario-style d12 spinner orby things, pretty and enticing, that you have to work for"
- [ ] Drafts stop auto-opening: spawn a CATCHABLE spinning d12 orb — grab = the draft; miss = it drifts away (next orb sooner). Bot must chase orbs.
- [x] **v1.22.4: THE DRAFT ORB** — drafts no longer auto-open at distance marks: a spinning d12 WONDER rides the current (lags to 0.8x so you can line up, sine-weaves, sheds rainbow sparkles, orbiting glints, halo). CATCH it = the draft opens (burst+ripple+unlock chime). MISS it = "the river keeps its gifts..." and the next orb comes at 45% of the normal gap (miss stings, never starves). True d12 voxel art: dodecahedron face-normal faceting, 12 iridescent hues, seams, glint — first render was "too big and too jank" (Scott, mid-build): now hi-res model crushed through the duck crisp-pixel pipeline (LANCZOS+palette-snap+outline) at 26px, x2 in-game. Never spawns during boss/thermals/forks/tutorial. Bot chases orbs (danger-weighted) — 3-run batch drafted 18 powers incl. a full campaign WIN. Timeline pips now mark orb arrivals. 🟡 catch one.
- [x] **v1.22.5: duckweed EXECUTED** (Scott: "get rid of this... it looks like puke") — env_duckweed_0/1 stripped from every ENV_TABLE pond mix + the menu drift set (stone swapped in). Art parked on disk unused. Purgatory keeps its bog via snag+stones.

## ➕ PLAYTEST (Scott, 2026-07-30): "switched up the music when we hit some pond — super jank"
- [ ] Pond-arrival music transition is a hard cut. Fix: real crossfade.
- [x] **v1.22.6** — TWO bugs: (1) the "seamless tint swap" premise was FALSE — only the 4 original tints share the 51.43s length; later pond tracks run 28.8-32s, so preserving playback position could call play() PAST the shorter track's end = the jank pond arrival (position now wraps via fmod against the new track's length); (2) no fade at all — now a true 1.6s crossfade on a second Music-bus player (old pond lingers underneath, new one rises; mid-fade re-swaps drop the oldest layer; boss cuts unchanged). Botsim run crossed multiple ponds clean. 🟡 ears check.

## ➕ TRIPLE (Scott, 2026-07-30)
- [x] **"i dont want ponds to have different music at all!" v1.22.7** — ONE RIVER ONE SONG: pond arrivals never change the track; the iconic theme plays everywhere (bosses + BOOMBOX wearable still interrupt; jukebox unaffected; crossfade machinery retained for those). THEME_MUSIC retired.
- [x] **"rustys words are so fuckin weird. he needs a redo" v1.22.7** — voice attempt #6: PLAIN WARM SINCERITY, grandpa-on-the-porch ("ah, my favorite duck. yes, i mean you." / "you're doing better than you think. you usually are." / "an old wing's secret: nobody knows what they're doing. fly anyway." / "proud of you, kid. that's all. carry on."). No mysticism, no bits, no drill. Thermals + snack lines matched. hawk_pool_v=5. NOTE if this misses: ask Scott for 2-3 example lines he DOES want.
- [x] **"get rid of Emerald Lake" v1.22.7** — full theme surgery (the Pleasant drill): THEMES/LOG_TINT/ENV_TABLE/BANK_PROPS/HERO_NAMES/THEME_MUSIC/water-wash all recut to 6; new order 0 Buker · 1 Woodbury · 2 Purgatory · 3 Sand · 4 Cochichewick · 5 Jimmy; Cochichewick hop-chime + Jimmy minnow checks shifted; theme_route [0..5] (the OOB bite was route arrays this time); three stale-labeled match blocks realigned (Emerald had been drawing a LAMPPOST since the Pleasant removal — bank/log/atmosphere accents now truly track their ponds); hero_emerald + bank_fern lore retired, art parked. Botsim + 9/9 shotsweep clean.
- [ ] **"lots of monotony just jumping and clearing logs until hitting the bosses"** — real design item, proposal pending.
- [x] → **v1.23.0 THE RIVER'S MOODS** (Scott: "idk but it gets boring" — ownership pass): diagnosis = two verbs (steer/hop), one rhythm (constant), between set-pieces. Building: LOW SKIM heron (the ANTI-hop), STEPPING-STONE log runs (logs become opportunity), river MOOD cycle (pressure/release breathing).
- [x] **v1.23.0 shipped**: (1) RIVER MOODS — quiet water cycles 14-22s micro-moods, never repeating: THE JAM (logs crowd 0.68x cadence), GLASS WATER (sparse + snack-rich breather), SKIMMERS, STONE RUNS; moods yield to bosses/forks/events/Rusty. (2) LOW SKIM — a heron crossing AT hop height: shadow-line telegraph + skree, then a low level crossing; grounded = clean pass underneath (+loft, "ducked under!"), airborne = a hit — THE ANTI-HOP verb. (3) STEPPING-STONE RUNS — 3-4 logs in a deliberate arc (frog riding the last): consecutive clears build a chiming combo (+N feathers each), frog-legs bounces count too — logs become opportunity. Bot: skim-aware (stays down), no skim deaths in 4-run batch; moodshot verified. 🟡 the boredom verdict is yours.
## ➕ PLAYTEST (Scott, 2026-08-08): "the game got boring tbh... it is very monotonous just log jumping all day"
- [ ] **THE BOREDOM, VERDICT ON v1.23.0: moods did NOT cure it.** Moods varied the log *pattern* but the duck still has one verb between set-pieces. Direction chosen by Scott 2026-08-08: THE DIVE + the river fights back (split below).
- [x] **THE DIVE — second core verb** v1.24.0 — SWIPE DOWN (or S/↓): guaranteed avoidance under EVERYTHING (logs, herons, skims, Chrissy, boss strikes/tides/globs) for a committed 1.5s breath; steering drops to 16% + the current sways you ("at the mercy"); surfacing under a log or Chrissy's hull = CLONK, your own bonk through the real hit pipeline (shield/duckling/death: "the river chooses where you come up"). Honest price: no snacks, no orb catch, loft auto-fire held while under; "slipped under!" +loft per log, clean multi-slip dives stoke ON FIRE heat; 3.5s "wet wings" cooldown ("wings dry!" tell). 🟡 in-motion feel = Scott's call.
- [x] **"absolutely seamless and cool"** v1.24.0 — the submerged duck draws in its own pass UNDER the log layer (she genuinely passes beneath everything); deep-teal refraction silhouette with worn gear still on her, wavering ghosts, pearl bubble trail, surface sheen, and THE PROMISE: a golden ring tightening on the exact surfacing spot (read it against the logs — the wager, drawn on the water). FILM-VERIFIED frame-by-frame via new `--diveshot [--clonk]` harness: both outcomes captured (slip-under + CLONK with log shatter). 🟡 the in-motion verdict is yours.
- [x] **Dive feels DISTINCT** v1.24.0 — its own state (not a hop variant): big sploosh in, world mutes its grabs, weak steering, bubbles, promise ring, burst out + quack. 🟡
- [x] **The river fights back** v1.24.0 — two new moods past 3km: WHITEWATER RAPIDS (announced; current +30%, chop shoulders you around, racing foam + bank churn) and WHIRLPOOLS (a spiral throat drifting the lane: caught = dragged to center, loft draining, steering forfeit — HOP out, DIVE under it clean, or ride it out). The verbs answer the water: logs punish dive-surfacing, whirlpools punish staying up. Truce-cleared at bosses/forks. Bot steers the rim + dives imminent throats. 🟡 feel.
- [x] **Tutorial updated** v1.24.0 — new beat after near-miss: "SWIPE DOWN to DIVE... slip under two logs", with coaching lines for hop-instead ("try SWIPING DOWN") and empty dives ("now TIME it"). Dive locked until Rusty teaches it. 🟡 needs a real hand on a real screen.
- [ ] Codex entries for THE DIVE / whirlpools / rapids — follow-up polish, not yet done.

## 🔧 SOLO (found via botsim, 2026-08-08)
- [x] **Chrissy killed 3/8 runs AT the first boss mark (5001ft)** — `_start_boss` cleared logs/enemies/globs but NOT Chrissy: if she was mid-weave when the boss arrived, she kept carving the lane through the intro = near-guaranteed shieldless death. Same gap in the fork truce, plus in-flight LOW SKIMs. New `_roamer_truce()` (turtle + skims + Chrissy) wired into boss start + fork spawn; Chrissy throttles off in character ("y'all got WEIRD water."), collision off. Botsim re-run: zero boss-mark deaths; her remaining kills are mid-river = working as designed.

- [x] **v1.23.1** (Scott: "the power up is flying under the logs") — the orb lived in the river-EVENTS layer (below logs/flotsam); now drawn in its own pass just beneath the duck — above every log. (_draw_river_events split: fork/events layer + _draw_draft_orb.) Orbshot + moodshot + botsim clean.
