...
- [ ] ⏳ AWAITING SCOTT on v1.21.46: reproduce the sustained-stutter stretch, screenshot VIEW APP LOGS. KEY READ: p2d big = our script; d2n big = engine-side (audio mixing on main thread is prime suspect — no-threads web build). If d2n: next lever is threads via coi-serviceworker on Pages (needs careful PWA testing) or trimming the audio graph.
