...
- [ ] **THE DIVE — second core verb** (Scott) — dive = you CERTAINLY go under and avoid everything, but you COMMIT: at the mercy of where you surface; surfacing under a log (or similar) = you bonk YOURSELF. Risk/reward, not a free pass.
- [ ] **"needs to be absolutely seamless and cool"** (Scott: "I have little faith in your artistic ability to pull this off well") — the bar: submerge/underwater/surface must read as one fluid motion, cool at every frame. Film-verify before claiming done.
- [ ] **Dive must feel DISTINCT** (Scott) — not a hop variant; its own motion, physics, sound, read.
- [ ] **The river fights back** (Scott co-pick) — water itself becomes gameplay (rapids/seams/whirlpools) — interleave with dive so the two answer each other.
- [ ] **Update the tutorial** (Scott) — teach the dive.
